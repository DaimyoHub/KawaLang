...
let type_mem_loc ctx = function
    Loc s -> (
        match LocalEnv.get ctx.vars s with
          None -> report_symbol_resolv (Loc_not_found s)
        | Some sd -> Ok sd.t
      )
  | e -> report_symbol_resolv (Not_loc e)
        ...
        match get_class_from_symbol ctx class_symbol with
          Ok cls -> (
            match get_method_from_class cls class_symbol with
              Ok ctor ->
                type_args ctx class_symbol (Cls class_symbol) args ctor.params
            | _ ->
            ...
            match get_class_from_symbol ctx class_symbol with
              Ok cls -> (
                match get_method_from_class cls callee with
                  Ok meth->
                    type_args ctx callee meth.ret_typ args meth.params
                | Error rep -> Error rep
              )
            | Error rep -> Error rep
          )
